...
public:
    ...
    /**
     * @brief 构建 Open API 实时数据 JSON（包含设备属性和 elementId）
     *
     * 与 buildRealtimeItem 不同，此方法：
     * - 用 device 对象包裹设备元数据
     * - elements 中包含 elementId（供指令下发使用）
     * - 附加 commandOperations / imageOperations
     */
    static Json::Value buildOpenApiRealtimeItem(
        const DeviceCache::CachedDevice& device,
        const RealtimeDataCache::DeviceRealtimeData& deviceData,
        const std::string& latestTime,
        const ConnectionChecker& isConnected = nullptr
        ...
        deviceObj["protocolType"] = device.protocolType;
        deviceObj["linkId"] = device.linkId;
        deviceObj["status"] = device.status;
        deviceObj["remoteControl"] = device.remoteControl;
                ...
                if (config.isMember("funcs") && config["funcs"].isArray()) {
                    ...
                    for (const auto& func : config["funcs"]) {
                        ...
                        if (hasJpegElement(func)) {
                            ...
                            parseUpElements(func, realtimeValues, addedGuideHex, elements, true);
                        }
                    }
                }
            } else if (device.protocolType == Constants::PROTOCOL_MODBUS) {
        ...
        // 操作定义（含 elementId，供指令下发使用）
        Json::Value commandOps, imageOps;
        parseProtocolFuncsStatic(device, commandOps, imageOps);
        if (!commandOps.empty()) {
