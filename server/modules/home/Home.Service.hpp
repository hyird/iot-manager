...
public:
    ...
    Task<Json::Value> getMonitorData() {
        ...
        if (auto modbusMetrics = dispatcher.getAdapterMetrics(Constants::PROTOCOL_MODBUS);
            modbusMetrics && modbusMetrics->available) {
            const auto totalResponses = modbusMetrics->stats["totalResponses"].asInt64();
            const auto crcErrors = modbusMetrics->stats["crcErrors"].asInt64();
            const auto timeouts = modbusMetrics->stats["timeouts"].asInt64();
            const auto exceptions = modbusMetrics->stats["exceptions"].asInt64();
            // 计算 Modbus 错误率
            double modbusErrorRate = totalResponses > 0
                ? (static_cast<double>(crcErrors + timeouts + exceptions) / totalResponses * 100.0)
                : 0.0;

            Json::Value modbus = modbusMetrics->stats;
            modbus["errorRate"] = modbusErrorRate;
            data["modbus"] = modbus;
        }

        // 6c. SL651 性能统计
        if (auto sl651Metrics = dispatcher.getAdapterMetrics(Constants::PROTOCOL_SL651);
            sl651Metrics && sl651Metrics->available) {
            const auto framesParsed = sl651Metrics->stats["framesParsed"].asInt64();
            const auto crcErrors = sl651Metrics->stats["crcErrors"].asInt64();
            const auto parseErrors = sl651Metrics->stats["parseErrors"].asInt64();
            // 计算 SL651 错误率
            double sl651ErrorRate = framesParsed > 0
                ? (static_cast<double>(crcErrors + parseErrors) / framesParsed * 100.0)
                : 0.0;

            Json::Value sl651 = sl651Metrics->stats;
...
private:
    ...
    static std::pair<double, double> getSystemMemory() {
        ...
        if (host_page_size(hostPort, &pageSize) != KERN_SUCCESS) {
            ...
            return {static_cast<double>(totalBytes) / (1024.0 * 1024.0), 0.0};
        }

        vm_statistics64_data_t vmStats{};
        mach_msg_type_number_t count = HOST_VM_INFO64_COUNT;
        if (host_statistics64(hostPort, HOST_VM_INFO64, reinterpret_cast<host_info64_t>(&vmStats), &count) !=
            KERN_SUCCESS) {
            mach_port_deallocate(mach_task_self(), hostPort);
            return {static_cast<double>(totalBytes) / (1024.0 * 1024.0), 0.0};
        }

        const auto availablePages = static_cast<uint64_t>(vmStats.free_count) +
            static_cast<uint64_t>(vmStats.inactive_count) +
            static_cast<uint64_t>(vmStats.speculative_count);
        const auto availableBytes = availablePages * static_cast<uint64_t>(pageSize);
        mach_port_deallocate(mach_task_self(), hostPort);
        const double total = static_cast<double>(totalBytes) / (1024.0 * 1024.0);
        const double available = static_cast<double>(availableBytes) / (1024.0 * 1024.0);
        const double used = std::clamp(total - available, 0.0, total);
        return {total, used};
#else
    ...
    static double getLoadAverage() {
        double loads[3]{};
        return getloadavg(loads, 1) == 1 ? loads[0] : 0.0;
    }
#endif
};
