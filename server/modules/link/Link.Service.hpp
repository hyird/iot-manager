...
public:
    ...
    Task<void> update(int id, const Json::Value& data) {
        ...
        if (data.isMember("agent_interface") ||
            data.isMember("agent_bind_ip") ||
            data.isMember("agent_prefix_length") ||
            data.isMember("agent_gateway")) {
            link.require(Link::agentBindingValid);
        }
    ...
    Task<void> remove(int id) {
        auto link = co_await Link::of(id);
        co_await link.require(Link::notReserved)
...
private:
    /**
     * @brief 从传输门面查询连接状态并注入到 Link
     */
    void injectConnectionStatus(Link& link) {
        auto status = LinkTransportFacade::instance().getStatus(link.id());
