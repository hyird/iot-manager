...
private:
    ...
    static Task<> createTables(const DbClientPtr& db) {
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_sys_role_code ON sys_role (code))");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_sys_user_username ON sys_user (username))");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_protocol_config_jsonb ON protocol_config USING GIN (config))");
    ...
    static Task<int> insertButton(const DbClientPtr& db, const std::string& name,
        ...
        co_return co_await insertMenu(db, name, parentId, "button", "", "", permissionCode, "", false, sortOrder);
    }
    ...
    static Task<> initializeMenus(const DbClientPtr& db, int roleId) {
        ...
        // 部门管理
        int deptId = co_await insertMenu(db, "部门管理", systemId, "page", "/system/department", "Dept", "", "ApartmentOutlined", false, 2);
        ...
        // 角色管理
        int roleMenuId = co_await insertMenu(db, "角色管理", systemId, "page", "/system/role", "Role", "", "TeamOutlined", false, 3);
        ...
        // 用户管理
        int userId = co_await insertMenu(db, "用户管理", systemId, "page", "/system/user", "User", "", "UserOutlined", false, 4);
