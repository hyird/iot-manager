...
private:
    ...
    static Task<> createTables(const DbClientPtr& db) {
        ...
        co_await db->execSqlCoro(R"(CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_node_code ON agent_node (code) WHERE deleted_at IS NULL)");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_agent_node_online ON agent_node (is_online))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_agent_node_deleted ON agent_node (deleted_at))");
        // 迁移：Agent 节点增加 secret 列（支持 UI 管理认证）
        co_await db->execSqlCoro(R"(ALTER TABLE agent_node ADD COLUMN IF NOT EXISTS secret VARCHAR(128))");
        // 迁移：Agent 节点增加 network_config 列（网口 IP/网关配置）
        co_await db->execSqlCoro(R"(ALTER TABLE agent_node ADD COLUMN IF NOT EXISTS network_config JSONB NOT NULL DEFAULT '[]'::jsonb)");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_agent_event_agent_time ON agent_event (agent_id, created_at DESC))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_agent_event_created ON agent_event (created_at DESC))");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_agent_endpoint_agent ON agent_endpoint (agent_id) WHERE deleted_at IS NULL)");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_agent_endpoint_deleted ON agent_endpoint (deleted_at))");

        // 迁移：从 device.protocol_params 中的网络字段迁移到 agent_endpoint 表
        // 使用 DO $$ 块检查 device 表是否存在（全新数据库时跳过）
        co_await db->execSqlCoro(R"(
            DO $$ BEGIN
        ...
        // 迁移：添加新列（必须在索引创建之前，旧数据库可能缺少这些列）
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS protocol VARCHAR(20) NOT NULL DEFAULT 'SL651')");
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS usage VARCHAR(20) NOT NULL DEFAULT 'device')");
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS agent_id INT NULL)");
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS agent_interface VARCHAR(100))");
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS agent_bind_ip VARCHAR(50))");
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS agent_prefix_length INT)");
        co_await db->execSqlCoro(R"(ALTER TABLE link ADD COLUMN IF NOT EXISTS agent_gateway VARCHAR(50))");
        // 索引
        ...
        co_await db->execSqlCoro(R"(CREATE UNIQUE INDEX IF NOT EXISTS idx_link_endpoint_route ON link (mode, ip, port, COALESCE(agent_id, 0), COALESCE(agent_bind_ip, '')) WHERE deleted_at IS NULL)");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_link_usage ON link (usage))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_link_agent_id ON link (agent_id) WHERE deleted_at IS NULL)");
        ...
        // 迁移：将现有 Agent 链路的连接信息回填到 device.protocol_params
        co_await db->execSqlCoro(R"(
            UPDATE device d SET
                link_id = 0,
        ...
        co_await db->execSqlCoro(R"(CREATE UNIQUE INDEX IF NOT EXISTS idx_open_access_key_hash ON open_access_key (access_key_hash))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_key_status ON open_access_key (status) WHERE deleted_at IS NULL)");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_key_expires ON open_access_key (expires_at) WHERE deleted_at IS NULL)");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_key_deleted ON open_access_key (deleted_at))");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_key_device_key ON open_access_key_device (access_key_id))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_key_device_device ON open_access_key_device (device_id))");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_webhook_access_key ON open_webhook (access_key_id) WHERE deleted_at IS NULL)");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_webhook_status ON open_webhook (status) WHERE deleted_at IS NULL)");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_webhook_deleted ON open_webhook (deleted_at))");
        co_await db->execSqlCoro(R"(CREATE UNIQUE INDEX IF NOT EXISTS idx_open_webhook_key_name ON open_webhook (access_key_id, name) WHERE deleted_at IS NULL)");
        ...
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_log_access_key ON open_access_log (access_key_id, created_at DESC))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_log_webhook ON open_access_log (webhook_id, created_at DESC))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_log_device ON open_access_log (device_id, created_at DESC))");
        co_await db->execSqlCoro(R"(CREATE INDEX IF NOT EXISTS idx_open_access_log_action ON open_access_log (action, created_at DESC))");
    ...
    static Task<> createTriggers(const DbClientPtr& db) {
        ...
        co_await db->execSqlCoro(R"(
            DO $$ BEGIN
                CREATE TRIGGER update_agent_node_updated_at BEFORE UPDATE ON agent_node
                    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
            EXCEPTION WHEN duplicate_object THEN null; END $$
        )");
        ...
        co_await db->execSqlCoro(R"(
            DO $$ BEGIN
                CREATE TRIGGER update_agent_endpoint_updated_at BEFORE UPDATE ON agent_endpoint
                    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
            EXCEPTION WHEN duplicate_object THEN null; END $$
        )");
        ...
        co_await db->execSqlCoro(R"(
            DO $$ BEGIN
                CREATE TRIGGER update_open_access_key_updated_at BEFORE UPDATE ON open_access_key
                    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
            EXCEPTION WHEN duplicate_object THEN null; END $$
        )");
        ...
        co_await db->execSqlCoro(R"(
            DO $$ BEGIN
                CREATE TRIGGER update_open_webhook_updated_at BEFORE UPDATE ON open_webhook
                    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
            EXCEPTION WHEN duplicate_object THEN null; END $$
        )");
    ...
    static Task<> initializeMenus(const DbClientPtr& db, int roleId) {
        ...
        // ==================== 开放接入 ====================
        int openAccessId = co_await insertMenu(
            db,
            "开放接入",
            0,
            Constants::MENU_TYPE_PAGE,
            "/iot/open-access",
            "OpenAccess",
            "",
            "CloudServerOutlined",
