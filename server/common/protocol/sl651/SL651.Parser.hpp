...
private:
    ...
    CommandCompletionCallback onCommandCompletion_;
...
public:
     ...
     * @brief 设置命令完成回调
     */
    void setCommandCompletionCallback(CommandCompletionCallback callback) {
        onCommandCompletion_ = std::move(callback);
    }
    ...
    /**
     * @brief 定期维护：清理过期多包会话和过大的链路缓冲区
     * 由 onMaintenanceTick 定期调用，确保即使没有新数据也能回收资源
     */
...
private:
    ...
    std::optional<ParsedFrameResult> buildFrameResult(int linkId, const Sl651Frame& frame,
        ...
        if (frame.direction == Direction::UP) {
            ParsedFrameResult::CommandCompletion completion;
            completion.commandKey = frame.remoteCode;
            completion.responseCode = frame.funcCode;
            completion.success = (frame.funcCode != FuncCodes::ACK_ERR);
            result.commandCompletion = completion;
        }
...
private:
    ...
    Task<void> onSingleFrameParsed(int linkId, const Sl651Frame& frame) {
        ...
        // 检查是否为命令完成事件（上行帧），在保存后通知以便关联响应 ID
        if (frame.direction == Direction::UP && onCommandCompletion_) {
            // 上行帧视为对下行指令的响应
            // E1 = 确认成功, E2 = 否认失败, 其他功能码的上行 = 对应指令的应答
            bool success = (frame.funcCode != FuncCodes::ACK_ERR);
            onCommandCompletion_(frame.remoteCode, frame.funcCode, success, recordId);
        }
    }
    ...
    /**
     * @brief 清理空闲链路缓冲区
     * 移除已空的缓冲区条目，避免 map 无限膨胀
     */
    void cleanStaleBuffers() {
        std::lock_guard<std::mutex> lock(bufferMutex_);
