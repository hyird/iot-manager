...
public:
    ...
    static AgentBridgeManager& instance() {
        static AgentBridgeManager inst;
        return inst;
    }
    ...
    Task<void> resetOnStartup(int eventRetentionDays = 30) {
        ...
        if (!resetResult.empty()) {
            LOG_INFO << "[AgentBridge] Startup: reset " << resetResult.size()
                     << " agent(s) to offline";
        }
        ...
        LOG_INFO << "[AgentBridge] Startup: cleaned events older than "
    ...
    void appendRecentEvent(int agentId,
        ...
        drogon::async_run([agentId, type, level, message, serializedDetail]() -> Task<> {
            try {
                ...
                LOG_WARN << "[AgentBridge] Persist agent event failed, agentId=" << agentId
