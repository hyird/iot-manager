...
public:
    ...
    /**
     * @brief 从数据库加载指定设备的最新数据到 Redis 缓存
     *
     * 不受 initialized 状态限制，可用于补加载 Redis 中缺失的设备数据。
     * 注意：此方法仅加载数据到 Redis，不设置 realtime:latest 键
     * （离线设备不应有 latest 标记）。
     */
    ...
    void clearLatestTime(int deviceId) {
        ...
        DrogonLoopSelector::getNext()->queueInLoop([deviceId]() {
