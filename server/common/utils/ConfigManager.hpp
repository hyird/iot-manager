...
private:
    ...
    static void validateAgent(const Json::Value& root,
                              std::vector<std::string>&,
                              std::vector<std::string>&) {
        // Agent 认证使用硬编码 secret，无需在 config 中配置 agent.nodes
        if (!root.isMember("custom_config") || !root["custom_config"].isObject()) {
            return;
        }
        const auto& custom = root["custom_config"];
        if (custom.isMember("agent") && !custom["agent"].isObject()) {
            // 仅在格式错误时提示（不阻断启动）
            LOG_WARN << "[Config] custom_config.agent 存在但不是对象，已忽略";
        }
    }
