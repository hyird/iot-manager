...
#ifdef _WIN32
// Windows 下不支持串口（EdgeNode 仅在 Linux ARM 上运行）
class EdgeNodeSerialLinkManager {
public:
    static EdgeNodeSerialLinkManager& instance() {
        static EdgeNodeSerialLinkManager inst;
        return inst;
    }
 ...
 * 使 EdgeNodeModbusPoller 可以透明地通过串口发送/接收数据。
 *
...
private:
    EdgeNodeSerialLinkManager() = default;
    ~EdgeNodeSerialLinkManager() { closeAll(); }
    EdgeNodeSerialLinkManager(const EdgeNodeSerialLinkManager&) = delete;
    EdgeNodeSerialLinkManager& operator=(const EdgeNodeSerialLinkManager&) = delete;
...
using SerialLinkManager = EdgeNodeSerialLinkManager;
