...
/**
 * @brief EdgeNode 端 Shell 会话管理器
 *
 * 通过 forkpty 创建 PTY，将 shell I/O 通过回调转发到 WebSocket 通道。
 * 同一时间只允许一个 shell 会话。
 */
class EdgeNodeShellManager {
public:
    using DataCallback = std::function<void(const std::string& data)>;
    using CloseCallback = std::function<void(int exitCode)>;

    EdgeNodeShellManager() = default;
    ~EdgeNodeShellManager() { close(); }

    EdgeNodeShellManager(const EdgeNodeShellManager&) = delete;
    EdgeNodeShellManager& operator=(const EdgeNodeShellManager&) = delete;
    ...
    std::string open(int cols, int rows, DataCallback onData, CloseCallback onClose) {
        ...
        std::cout << "[EdgeNodeShell] opened: pid=" << pid
...
using AgentShellManager = EdgeNodeShellManager;
