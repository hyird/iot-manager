 ...
 */
class EdgeNodeL2ConfigBridge {
public:
    /// @brief 发现回调，参数为收到请求的接口 MAC，返回 JSON payload
    using DiscoverHandler = std::function<std::string(const l2config::MacAddr& localMac)>;

    EdgeNodeL2ConfigBridge() = default;
    ~EdgeNodeL2ConfigBridge() { stop(); }

    EdgeNodeL2ConfigBridge(const EdgeNodeL2ConfigBridge&) = delete;
    EdgeNodeL2ConfigBridge& operator=(const EdgeNodeL2ConfigBridge&) = delete;
...
#ifndef __linux__
        return "EdgeNodeL2ConfigBridge only supported on Linux";
#else
        if (running_) return "";
...
using L2ConfigBridge = EdgeNodeL2ConfigBridge;
