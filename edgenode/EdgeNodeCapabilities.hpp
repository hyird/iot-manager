...
class EdgeNodeCapabilitiesCollector {
public:
    static Json::Value collectCapabilities() {
        ...
        capabilities["network_backend"] = EdgeNodeNetworkConfigurator::backendName(
            EdgeNodeNetworkConfigurator::detectBackend());
        return capabilities;
    }
...
private:
    ...
    static std::string detectInterfaceMethod([[maybe_unused]] const std::string& name) {
#ifdef __linux__
        const auto backend = EdgeNodeNetworkConfigurator::detectBackend();

        if (backend == EdgeNodeNetworkConfigurator::Backend::NETWORKD) {
        ...
        if (backend == EdgeNodeNetworkConfigurator::Backend::NETPLAN) {
        ...
        if (backend == EdgeNodeNetworkConfigurator::Backend::IFUPDOWN) {
            for (const auto& path : {
...
using AgentCapabilitiesCollector = EdgeNodeCapabilitiesCollector;
