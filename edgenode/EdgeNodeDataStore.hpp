...
public:
    ...
    bool initialize(const std::string& dbPath = "./data/agent_cache.db") {
        ...
        if (rc != SQLITE_OK) {
            std::cout << "[ERROR] " << "[EdgeNodeDataStore] Failed to open SQLite: " << sqlite3_errmsg(db_) << std::endl;
            db_ = nullptr;
            return false;
        }
        ...
        std::cout << "[EdgeNodeDataStore] Initialized: " << dbPath << std::endl;
        return true;
    }
    ...
    void storeBatch(const std::vector<ParsedFrameResult>& results) {
        ...
        std::cout << "[EdgeNodeDataStore] stored " << results.size() << " record(s)" << std::endl;
    }
...
private:
    void store(const ParsedFrameResult& r) {
        ...
        if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
            std::cout << "[ERROR] " << "[EdgeNodeDataStore] store prepare failed: " << sqlite3_errmsg(db_) << std::endl;
            return;
        }
        ...
        if (sqlite3_step(stmt) != SQLITE_DONE) {
            std::cout << "[ERROR] " << "[EdgeNodeDataStore] store failed: " << sqlite3_errmsg(db_) << std::endl;
        }
        sqlite3_finalize(stmt);
    }
    ...
    void exec(const std::string& sql) {
        ...
        if (sqlite3_exec(db_, sql.c_str(), nullptr, nullptr, &errMsg) != SQLITE_OK) {
            std::cout << "[ERROR] " << "[EdgeNodeDataStore] exec failed: " << (errMsg ? errMsg : "unknown") << std::endl;
...
using AgentDataStore = EdgeNodeDataStore;
