        ...
        for (const auto& ep : modbusEndpoints) {
            for (const auto& dev : ep.devices) {
                ...
                if (!ctx.readGroups.empty()) {
                    std::cout << "[AgentModbusPoller] device: id=" << ctx.deviceId
                              << ", name=" << ctx.deviceName
                    ...
                    std::cout << "[WARN] " << "[AgentModbusPoller] device " << dev.id
                              << " (" << dev.name << ") has no valid read groups, skipping" << std::endl;
                }
            }
        }

        std::cout << "[AgentModbusPoller] Loaded " << devices_.size() << " Modbus device(s)" << std::endl;
    }
...
private:
    ...
    void flushPollCycle(DevicePollContext& ctx) {
        std::cout << "[AgentModbusPoller] poll cycle complete: device=" << ctx.deviceId
                  << " (" << ctx.deviceName << ")"
