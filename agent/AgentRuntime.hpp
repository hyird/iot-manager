...
#include "AgentDataStore.hpp"
#include "AgentModbusPoller.hpp"
#include "AgentNetworkConfigurator.hpp"
#include "AgentProtocolEngine.hpp"
#include "AgentShellManager.hpp"
#include "L2ConfigBridge.hpp"
...
public:
    ...
    void run() {
        ...
        if (existingLoop) {
            std::cout << "[Agent] reusing existing EventLoop on main thread" << std::endl;
            loopOwned_.reset();
            loop_ = existingLoop;
        } else {
            loopOwned_ = std::make_unique<trantor::EventLoop>();
            loop_ = loopOwned_.get();
            std::cout << "[Agent] event loop created" << std::endl;
        }

        // 初始化本地 SQLite 缓存
        if (!dataStore_.initialize("./data/agent_cache.db")) {
            std::cerr << "[Agent] WARNING: Failed to initialize local data store" << std::endl;
        }
        ...
        // 串口链路回调
        SerialLinkManager::instance().setDataCallback(
            [this](int epKey, const std::string& data) {
                onEndpointData(epKey, /*clientAddr=*/"serial", data);
            }
        );
        SerialLinkManager::instance().setConnectionCallback(
        ...
        modbusPoller_.setSendCallback([](int endpointKey, const std::string& data) -> bool {
            if (SerialLinkManager::instance().isOpen(endpointKey)) {
                return SerialLinkManager::instance().sendData(endpointKey, data);
            }
        ...
        std::cout << "[Agent] connecting to " << config_.platformHost << "/agent/ws ..." << std::endl;
    ...
    void stop() {
        ...
        loop_->queueInLoop([this]() {
            ...
            SerialLinkManager::instance().closeAll();
            dataStore_.close();
            loop_->quit();
        });
    }
...
private:
    ...
    void connectPlatform() {
        ...
        wsClient_->setConnectionClosedHandler([this](const drogon::WebSocketClientPtr&) {
            std::cout << "[WARN] " << "[Agent] WebSocket connection closed" << std::endl;
            connected_ = false;
            agentId_ = 0;
            scheduleReconnect();
        });
        ...
        wsClient_->connectToServer(
            ...
            [this](drogon::ReqResult result, const drogon::HttpResponsePtr& resp, const drogon::WebSocketClientPtr&) {
                if (result != drogon::ReqResult::Ok) {
                    std::cout << "[Agent] connect failed (code=" << static_cast<int>(result)
    ...
    void applyEndpoint(const EndpointRuntime& runtime) {
        ...
        if (ep.transport == "serial") {
            SerialLinkManager::instance().openPort(
                runtime.key,
                "ep:" + ep.id,
                ep.channel,
                ep.baudRate
    ...
    void deactivateEndpoint(const EndpointRuntime& runtime) {
        if (runtime.endpoint.transport == "serial") {
            SerialLinkManager::instance().closePort(runtime.key);
        } else {
...
private:
    ...
    // 协议引擎 + 数据缓存
    AgentProtocolEngine protocolEngine_;
    AgentModbusPoller modbusPoller_;
    AgentDataStore dataStore_;
    std::vector<agent::DeviceEndpoint> currentEndpoints_;  // 当前端点配置（含 protocolConfig）

    // Shell
    AgentShellManager shellManager_;

    // L2 二层配置
    L2ConfigBridge l2Bridge_;
