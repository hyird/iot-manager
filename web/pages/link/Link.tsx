    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (record.mode === "TCP Server" && status === "listening") {
          ...
          const clientTag = (
            <Tag color="blue" style={{ cursor: count > 0 ? "pointer" : "default" }}>
              {count} 客户端
            </Tag>
          );
            ...
            <Space size={4}>
              ...
              {count > 0 ? (
                <Tooltip
                  title={
                    <div>
                      <div style={{ marginBottom: 4, fontWeight: 500 }}>已连接客户端：</div>
                      {clients.map((ip, idx) => (
                        <div key={idx}>{ip}</div>
                      ))}
                    </div>
    ...
    >
      ...
      >
        <Form<LinkFormValues> form={form} layout="vertical" onFinish={onFinish}>
          ...
          >
            <InputNumber placeholder="如: 8080" style={{ width: "100%" }} min={1} max={65535} />
          </Form.Item>
