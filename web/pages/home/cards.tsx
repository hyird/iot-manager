...
export const ServerResourceCard = memo(function ServerResourceCard({
  ...
  const hasSystemMemory = (monitor?.server.memoryTotal ?? 0) > 0;
  const hasProcessMemory = (monitor?.server.processMemory ?? 0) > 0;
        ...
        <div className="space-y-3">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Text strong>
                {hasSystemMemory
                  ? `${formatMemory(monitor?.server.memoryUsed ?? 0)} / ${formatMemory(
                      monitor?.server.memoryTotal ?? 0
                    )}`
                  : "N/A"}
              </Text>
            </div>
            <Progress
              percent={hasSystemMemory ? memPercent : 0}
              strokeColor={hasSystemMemory ? getStatusColor(memPercent) : "#d9d9d9"}
              size="small"
              showInfo={hasSystemMemory}
            />
          </div>
          ...
          <InfoRow
            ...
            value={hasProcessMemory ? formatMemory(monitor?.server.processMemory ?? 0) : "N/A"}
          />
          <Divider className="!my-2" />
          <InfoRow
            label="主机名"
                          ...
                          <>
                            ...
                            <div className="grid grid-cols-3 gap-2 text-center mt-2">
                              <div>
                                ...
                                <Text className="text-sm">
                                  {formatNumber(monitor.modbus.registryDtuCount ?? 0)}
                                  {" / "}
                                  {formatNumber(monitor.modbus.registryDeviceCount ?? 0)}
                                </Text>
                              </div>
                              <div>
                                <Text type="secondary" className="text-xs block">
                                ...
                                <Text
                                  ...
                                  style={{
                                    color:
                                      (monitor.modbus.sessionCount ?? 0) > 0 &&
                                      (monitor.modbus.boundSessionCount ?? 0) === 0
                                        ? "#ff4d4f"
                                        : undefined,
                                  }}
                                >
                                  {formatNumber(monitor.modbus.boundSessionCount ?? 0)}
                                  {" / "}
                                  {formatNumber(monitor.modbus.sessionCount ?? 0)}
                                </Text>
                              </div>
                              <div>
                                <Text type="secondary" className="text-xs block">
                                ...
                                <Text className="text-sm">
                                  {formatNumber(monitor.modbus.onlineRouteCount ?? 0)}
                                </Text>
                              </div>
                            </div>
                            <div className="grid grid-cols-3 gap-2 text-center mt-2">
                              <div>
                                ...
                                <Text className="text-sm">
                                  {formatNumber(monitor.modbus.probingSessionCount ?? 0)}
                                  {" / "}
                                  {formatNumber(monitor.modbus.inflightSessionCount ?? 0)}
                                </Text>
                              </div>
                              <div>
                                <Text type="secondary" className="text-xs block">
                                ...
                                <Text
                                  ...
                                  style={{
                                    color:
                                      (monitor.modbus.queuedJobCount ?? 0) > 0
                                        ? "#faad14"
                                        : undefined,
                                  }}
                                >
                                  {formatNumber(monitor.modbus.queuedJobCount ?? 0)}
                                </Text>
                              </div>
                              <div>
                                <Text type="secondary" className="text-xs block">
                                ...
                                <Text className="text-sm">
                                  {formatNumber(monitor.modbus.legacyMappingsCleanedTotal ?? 0)}
                                </Text>
                              </div>
                            </div>
                          </>
